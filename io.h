    ...
    match(value)
    {
        ...
        MatchInt(value, i)
        {
            if (i == 0) {
                ...
                if (i < 0) waks_io_print(from_cstr("-"));
                while (num > 0) {
        ...
        MatchPtr(value, ptr)
        {
            if (ptr == WAKS_NOVALUE) waks_io_print(from_cstr("Invalid memory not allocated: ptr (null)"));
            waks_io_print(from_cstr("Memory allocated successfully: ptr (Ox)"));
...
void dbg_print_(waks_string str)
{
