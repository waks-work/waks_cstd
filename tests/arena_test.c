...
void test_vector() {
    Arena *arena = ArenaAlloc((waks_u64)(GB((1))));
...
void linked_list_test() {
    ...
    TEST("List initialisation ,push_back and remove") {
        ...
        TEST_ASSERT(list_is_empty(line_list),
                    "New list handle should be NULL (empty)");
        ...
        TEST_ASSERT(line_1->node.next == NULL,
                    "Single-node list tail 'next' must be NULL");
        ...
        TEST_ASSERT(line_2->node.next == NULL,
                    "New tail end of the list must point to NULL");
        ...
        TEST_ASSERT(line_2->node.prev == NULL,
                    "New head 'prev' must be NULL after removal");
...
void run_error_handling_suite() {
    ...
    TEST("Option test method functionality") {
        ...
        TEST_ASSERT(opt_some.has_value,
                    "Option(Some) should have has_value set to true");
        TEST_ASSERT(!opt_none.has_value,
                    "Option(None) should have has_value set to false");
...
void test_handle_safety() {
    ...
    TEST("Testing handle defer safety") {
        ...
        TEST_ASSERT(ptr3 == NULL,
                    "Handle must be invalid after ArenaReset processes defers");
    }
...
void _start() {
    ...
    // Exit with 0.
    waks_syscall6(SYS_exit, 0, 0, 0, 0, 0, 0);
}
